...
class LeadController {
    async createLead(req, res) {
        try {
            const { company_name, contact_person, phone, email, address, city, state, 
                    gst_number, lead_source, priority, expected_close_date, followup_date, 
                    notes, assigned_to, products } = req.body;
            ...
            // Generate lead ID
            const [lastLead] = await pool.query('SELECT lead_id FROM leads ORDER BY id DESC LIMIT 1');
            let leadId = 'L001';
            ...
            try {
                ...
                const [result] = await connection.query(
                    `INSERT INTO leads (lead_id, company_name, contact_person, phone, email, address, 
                     ...
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'New')`,
                    [leadId, company_name, contact_person, phone, email, address, city, state,
                     gst_number, lead_source, priority, expected_close_date, followup_date,
                     notes, assigned_to || req.user.id, req.user.id]
                );
                    ...
                    for (const product of products) {
                        await connection.query(
                            `INSERT INTO lead_products (lead_id, product_id, product_name, variant, 
                             ...
                             VALUES (?, ?, ?, ?, ?, ?, ?)`,
                            [result.insertId, product.product_id || null, product.product_name,
                             product.variant, product.quantity, product.unit_price,
    ...
    async updateLead(req, res) {
        try {
            ...
            const allowedFields = ['company_name', 'contact_person', 'phone', 'email', 'address',
                                   'city', 'state', 'priority', 'status', 'expected_close_date',
