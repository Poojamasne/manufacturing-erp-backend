...
class ReportsController {
    ...
    try {
        ...
        // 2. Get Lead Sources Distribution (FIXED - with date filter)
        let sourceQuery = `
            SELECT 
        ...
        if (dateCondition) {
            // Replace created_at with leads.created_at
            let leadDateCond = dateCondition.replace(/created_at/g, 'leads.created_at');
            sourceQuery += ` ${leadDateCond}`;
        }
        sourceQuery += ` GROUP BY lead_source ORDER BY value DESC`;
        
        const [sourceData] = await pool.query(sourceQuery, dateParams);
        console.log('Source Data:', sourceData);
        
        // 3. Get KPI Statistics (FIXED - with date filter)
        // Total leads count with date filter
        let leadsQuery = `SELECT COUNT(DISTINCT id) as total_leads FROM leads WHERE 1=1`;
        if (dateCondition) {
            let leadDateCond = dateCondition.replace(/created_at/g, 'created_at');
            leadsQuery += ` ${leadDateCond}`;
        }
        ...
        // 5. Get Sales Leaderboard (FIXED - with date filter)
        ...
        if (dateCondition) {
            // Apply date filter to both leads and orders
            let leadDateCond = dateCondition.replace(/created_at/g, 'l.created_at');
            let orderDateCond = dateCondition.replace(/created_at/g, 'o.created_at');
            leaderboardQuery += ` ${leadDateCond} AND (o.id IS NULL OR ${orderDateCond.substring(4)})`; // Remove the AND part
        }
        
        leaderboardQuery += ` GROUP BY l.assigned_to, u.name ORDER BY revenue DESC LIMIT 10`;
        
        const [leaderboardData] = await pool.query(leaderboardQuery, [...dateParams, ...dateParams]);
